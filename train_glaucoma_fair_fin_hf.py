...
if __name__ == '__main__':
    ...
    if args.modality_types != 'rnflt':
        print("NOT IMPLEMENTED LOOK AT CODE COMMENTED ABOVE TO FIX")
        exit(0)
    in_dim = 1
    extra_info = {
        'in_feat_to_final': in_feat_to_final,
        'normalization_type': args.normalization_type,
    }
    if args.normalization_type == 'fin':
        extra_info['ag_norm_params'] = {"num_attr": imb_info.no_of_attr, "dim": in_feat_to_final, "mu" : args.fin_mu, "sigma" : args.fin_sigma, "momentum" : args.fin_momentum}
    elif args.normalization_type == 'lbn':
        extra_info['ag_norm_params'] = {"dim": in_feat_to_final}
    elif args.normalization_type == 'bn':
        extra_info['ag_norm_params'] = {"dim": in_feat_to_final}
    model = create_model(model_type=args.model_type, in_dim=in_dim, out_dim=out_dim, extra_info=extra_info)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if best_auc <= test_auc:
            ...
            if args.save_weights:
                model.save_pretrained(os.path.join(args.result_dir, "best_model"))
